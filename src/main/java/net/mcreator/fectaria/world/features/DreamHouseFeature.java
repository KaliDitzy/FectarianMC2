...
package net.mcreator.fectaria.world.features;
...
public class DreamHouseFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(ResourceKey.create(Registries.DIMENSION, new ResourceLocation("fectaria:dream")));
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public DreamHouseFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("fectaria", "dream_house"));
		if (template == null)
			return false;
